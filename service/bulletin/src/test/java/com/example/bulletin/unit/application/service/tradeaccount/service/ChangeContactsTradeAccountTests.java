...
public class ChangeContactsTradeAccountTests {
    ...
    public void shouldThrowWhenTradeAccountNotFound() {
        // Arrange
        ChangeContactsTradeAccountRequest request = createRequest();
        when(tradeAccountRepository.findById(any(UUID.class)))
                .thenReturn(Optional.empty());

        // Act & Assert
        assertThrows(ResourceNotFoundException.class, () -> service.changeContacts(request));
    }

    @Test
    public void shouldChangeContactsAndSave() {
        // Arrange
        ChangeContactsTradeAccountRequest request = createRequest();

        // Act
        service.changeContacts(request);

        // Assert
        verify(tradeAccountRepository).save(tradeAccountCaptor.capture());
        TradeAccount actual = tradeAccountCaptor.getValue();

        assertThat(actual.getContacts()).isEqualTo(request.getContacts());
    }
    ...
    public void shouldMapBeforeReturn() {
        ...
        // Act
        ChangeContactsTradeAccountResponse response = service.changeContacts(request);
    ...
    private ChangeContactsTradeAccountRequest createRequest() {
        return ChangeContactsTradeAccountRequest.builder()
                .id(UUID.randomUUID())
                .contacts("Telegram: @test, Email: test@example.com")
                .build();
    }
