...
import com.example.bulletin.application.service.tradeaccount.data.response.SetApproximateLocationTradeAccountResponse;
...
public class SetApproximateLocationTests {
    ...
    public void shouldThrowWhenTradeAccountNotFound() {
        // Arrange
        SetApproximateLocationTradeAccountRequest request = createRequest();
        when(tradeAccountRepository.findById(any(UUID.class)))
                .thenReturn(Optional.empty());

        // Act & Assert
        assertThrows(ResourceNotFoundException.class, () -> service.setApproximateLocation(request));
    }

    @Test
    public void shouldSetApproximateLocationAndSave() {
        // Arrange
        SetApproximateLocationTradeAccountRequest request = createRequest();

        // Act
        service.setApproximateLocation(request);
        ...
        assertThat(actual.isCoordinatesExact()).isFalse();
    }
    ...
    public void shouldMapBeforeReturn() {
        ...
        // Act
        SetApproximateLocationTradeAccountResponse response = service.setApproximateLocation(request);
    ...
    private SetApproximateLocationTradeAccountRequest createRequest() {
